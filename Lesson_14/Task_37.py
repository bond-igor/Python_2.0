...
class TestEmployee:
	...
	def test_employee_full_name(self, p):
		assert p.full_name() == 'Ivanov Ivan Ivanovich'

	def test_employee_birthday(self, p):
		p.birthday()
		assert p.get_age() == 31

	def test_employee_raise_salary(self, p):
		p.raise_salary(10)
		assert p.salary == 55000.0

	def test_employee_str(self, p):
		assert str(p) == 'Ivanov Ivan Ivanovich (Manager)'

	def test_employee_last_name_title(self, p):
		assert p.last_name == 'Ivanov'
